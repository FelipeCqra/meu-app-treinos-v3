...
function mudarAba(targetTab) {
    ...
    const btnAtivo = document.querySelector(`.nav-item[data-target="${targetTab}"]`);
...
document.querySelectorAll('.nav-item').forEach(btn => {
    btn.addEventListener('click', () => mudarAba(btn.getAttribute('data-target')));
});
...
function iniciarEdicao(sessao) {
    ...
    document.querySelector('#tab-registrar h1').textContent = "✏️ Editando Treino";
    document.querySelector('#btn-salvar-treino .btn-text').textContent = "ATUALIZAR SESSÃO";
    document.getElementById('btn-cancelar-edicao').classList.remove('hidden');

    document.getElementById('semana').value = sessao.semana || sessao.semanaNum || 1;
...
function resetarModoFormulario() {
    ...
    document.querySelector('#btn-salvar-treino .btn-text').textContent = "SALVAR SESSÃO";
...
async function handleFormSubmit(e) {
    ...
    exerciseBlocks.forEach(block => {
        ...
        if (seriesExtraidas.length === 0) {
            mostrarNotificacao(`Adicione pelo menos 1 série para "${name}"!`, "warning");
            erroValidacao = true;
            return;
        }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnAddExercise && container) {
        btnAddExercise.addEventListener("click", () => renderNovaLinhaExercicio(container, obterMembroAtual()));
    }
